...
class SearchRequest(BaseModel):
    ...
    # BUG FIX: N8N sends unset parameters as the string "null" instead of JSON null.
    # Without these validators, Pydantic raises a validation error (which surfaces as
    # a 500 rather than 422 if there's no global exception handler).
    @field_validator("salary_range", mode="before")
    @classmethod
    def parse_salary_range(cls, v):
    ...
    @classmethod
    def parse_location_filter(cls, v):
        if v == "null" or v == "":
            return None
        return v
